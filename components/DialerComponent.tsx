...
const DialerComponent: React.FC<DialerComponentProps> = ({
  ...
  const handleModalSubmit = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.post<CallResponse>('/api/make-call', {
        contact: {
        ...
        userId,
        credentials: { twilioSid, twilioAuthToken, vapiKey },
        agentSettings: {
      ...
      if (response.data.listenUrl) {
        setCallStatus({
          isActive: true,
          listenUrl: response.data.listenUrl,
  ...
  return (
    ...
    {/* Modal for Live Call */}
    {callStatus.isActive && callStatus.listenUrl && (
      <LiveCallModal
        listenUrl={callStatus.listenUrl}
        onClose={() => {
