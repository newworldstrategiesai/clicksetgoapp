...
type ChatUser = (typeof conversations)[number] | AIAgent;
...
export default function Chats() {
  ...
  useEffect(() => {
    ...
    if (storedThreads) {
      ...
      setSelectedUser(parsedThreads[0] || conversations[0]); // Fallback to default
    } else {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    // Show the loading '...' message in the chat
    ...
    // Fetch AI response after user message
    try {
      ...
      // Update thread with AI response, replacing the loading '...' message
      setThreads(prev => {
        const updatedThreads = prev.map(thread => {
          if (thread.id === selectedUser.id) {
            return {
              ...
              messages: [{
                ...
                timestamp: new Date().toISOString()
              }, ...thread.messages.slice(1)] // Replace '...' message with AI response
            };
          }
          return thread;
        });
