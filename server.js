...
app.prepare().then(() => {
      ...
      Papa.parse(fileContents, {
        ...
        complete: (results) => {
          ...
          const contacts = results.data.map(contact => ({
            ...
            phone: contact['Phone'].replace(/\D/g, ''), // Normalize phone number
  ...
  server.post('/vapi-webhook', async (req, res) => {
    const data = req.body;
    ...
    const phoneNumber = data.message?.call?.customer?.number.replace(/\D/g, ''); // Normalize phone number
    console.log('Extracted phone number:', phoneNumber); // Log the extracted phone number
    ...
    console.log('Determined first name:', firstName); // Log the determined first name
  ...
  server.listen(PORT, async (err) => {
    ...
    // Start Ngrok
    try {
      const url = await ngrok.connect(PORT); // Connect Ngrok to port 3000
